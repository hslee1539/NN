...
def backward(dout_array, w_array, w_shape, dx_array):
    ...
    for dx_index in range(len(dx_array)):
        pass_dout_index = dx_index // w_shape[0] * w_shape[1]
        pass_w_index = dx_index % w_shape[1]

        dx_array[dx_index] = 0
        for product_index in range(w_shape[1]):
            dx_array[dx_index] += dout_array[(pass_dout_index + product_index) % dout_size] * w_array[(pass_w_index + product_index) % w_size]
    return None
...
def partialBackward(dout_array, w_array, w_shape, dx_array, index, max_index):
    ...
    for dx_index in range(index * len(dx_array) // max_index, (index + 1) * len(dx_array) // max_index):
        ...
        tmp = zero
        for product_index in product_index_range:
            tmp += dout_array[pass_dout_index + product_index] * w_array[pass_w_index + product_index]
...
def backward_dw(x_array, x_shape, dout_array, dw_array, dw_shape):
    zero = type(dw_array[0])(0)
    x0_range = range(x_shape[0])
    for dw_index in range(len(dw_array)):
        dw1 = dw_index % dw_shape[1]
        dw0 = dw_index // dw_shape[1]
        tmp = zero
        for x0 in x0_range:
            tmp += x_array[x0 * x_shape[1] + dw0] * dout_array[x0 * dw_shape[1] + dw1]
        dw_array[dw_index] = tmp

    
def backward_db(dout_array, dbias_array):
    zero = type(dbias_array[0])(0)
    dbias_len = len(dbias_array)
    d0_range = range(len(dout_array) // dbias_len)

    for dbias_index in range(dbias_len): 
        tmp = zero
        for d0 in d0_range:
